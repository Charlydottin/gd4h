...
from utils import cast_type_given_rule
...
def import_organizations():
    ...
    org_doc = os.path.abspath(
        os.path.join(data_dir, "organizations", "organizations_fr.csv")
    )
    ...
    with open(org_doc, "r") as f:
        ...
        for row in reader:
            org = {"fr": {}, "en": {}}
            org["fr"] = row
            org["en"] = translate_doc("organization", org, _from="fr")
...
def import_datasets(rebuild=False):
    datasets_doc = os.path.abspath(
        os.path.join(data_dir, "datasets", "datasets_fr.csv")
    )
    ...
    with open(datasets_doc, "r") as f:
        ...
        for row in reader:
            dataset = {"fr": {}, "en": {}}
            dataset["fr"] = {
                k: cast_type_given_rule("dataset", k, v) for k, v in row.items()
            }
            dataset["en"] = translate_doc("dataset", dataset, _from="fr")
