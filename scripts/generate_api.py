...
from utils import get_rules, get_rules
...
def get_references():
    ...
    for rule in DB.rules.find(
        {"constraint": "enum", "is_controled": True, "reference_table": {"$ne": ""}}
    ):
        ...
        for record in DB[ref_name].find({}):
            references[ref_model_name].append(
                {"en": record["name_en"], "fr": record["name_fr"]}
            )
...
def get_filter_model(model_name="dataset"):
    ...
    info = {}
    model_rules = list(
        DB["rules"].find({"model": model_name, "is_facet": True}, {"_id": 0})
    )
    for lang in AVAILABLE_LANG:
        ...
        for rule in model_rules:
            ...
            if rule["multiple"]:
                if rule["reference_table"] != "":
                    py_type = (
                        rule["reference_table"].replace("ref_", "").title()
                        + "Enum"
                        + lang.title()
                    )
                    external_refs.append(("reference", py_type))
                elif rule["external_model"] != "":
                    external_model_rules = list(
                        DB.rules.find(
                            {"model": rule["external_model"]},
                            {"_id": 0, "translation": 1},
                        )
                    )
                    is_multilang_ext_model = any(
                        [r["translation"] for r in external_model_rules]
                    )
                ...
                if rule["reference_table"] != "":
                    ...
                    external_model_rules = [
                        DB.rules.find({"model": rule["external_model"]})
                    ]
                    is_multilang_ext_model = any(
                        [r["translation"] for r in external_model_rules]
                    )
...
def get_model(model_name="dataset"):
    ...
    if is_multilang_model:
        ...
        for rule in model_rules:
            ...
            info[model_title].append(line)
    external_models = set(
        [n for n in external_models if n[0] != model_name and n[0] != "reference"]
    )
    model_info.append(("import_model", list(external_models)))
    model_info.append(("import_reference", list(set(external_refs))))
    model_info.extend(list(info.items()))
...
def get_filters():
    ...
    for facet in facet_rules:
        ...
        is_controled, is_multiple, is_bool = (
            facet["is_controled"],
            facet["multiple"],
            facet["datatype"] == "boolean",
        )
        filter_d = {
            "name": field_name,
