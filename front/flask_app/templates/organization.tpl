...
<div class="fr-container fr-container--fluid fr-py-4w">
  ...
  <div class="fr-grid-row fr-grid-row--gutters">
    ...
    <div class="fr-col">
    <h3 class="fr-grid-row fr-grid-row--gutters">{{org["name"]}}</h3>
    <h4>{{org["acronym"]}}</h4>
    <p class="label-fr"> URL: <a href="{{org["url"]}}" class="" target="_blank">{{org["url"]}}</a></p>
    
    <p>{{org["about"]}}</p>
    <ul class="fr-btns-group fr-col-4">
    {% if "agent_type" in org %}
        <li><a href="references/agent_type" class="tag-fr fr-btn blue-france-300">{{org["agent_type"]["label_en"]}}</a></li>
    {%endif%}
    {% if "organization_type" in org %}
        <li><a href="/references/organization_type" class="fr-btn blue-france-300">{{org["organization_type"]["label_en"]}}</a></li>
    {%endif%}
    </ul>
    <div >
    <p>{{count}} dataset {%if count != "1" %}s{%endif%} online:</p>
    </div>
    <ul>
    {%for dataset in datasets%}
    <li><a href="/datasets/{{dataset['id']}}/">{{dataset["name"]}}</a></li>
    {%endfor%}
    </ul>
</div>
    </div>
...
<ul class="fr-btns-group fr-btns-group--inline">
    <li>
    <button class="fr-btn fr-btn--secondary fr-fi-error-warning-line fr-link--icon-left" onclick="location.href='https://github.com/c24b/gd4h/issues/new?assignees=c24b&labels=question&template=general-comment.md&title=%5BCOMMENT%5D'" title="Report and error">Report an error</button>
    </li>
    <li>
    <button onclick="location.href='https://github.com/c24b/gd4h/issues/new?assignees=c24b&labels=bug&template=default.md&title=%23Error%20%5BORG%5D%20{{org["name"]}}'" class="fr-btn fr-fi-chat-quote-line fr-btn--icon-right" title="Make a comment">
              Make a  Comment
    </button>
    </li>
